...
export class WritingSessionTracker {
  ...
  constructor(
    ...
    this.unsubscribeDrafts = draftsStore.subscribe(
      this.countWordsInDrafts.bind(this)
    );

    this.debouncedCountDraft = debounce(
      this.countDraftContaining.bind(this),
      1000
    );
    ...
    this.unsubscribeActiveWordCount = activeFileWordCountStatus.subscribe(
      (status) => {
        if (status) {
  ...
  private async countWordsInDraft(
    draft: Draft
  ...
  private async countDraftContaining(
    file: TAbstractFile,
    oldPath: string | null
  ) {
    const draft = draftForPath(file.path, get(draftsStore), this.vault);
    if (draft) {
    ...
    if (oldPath) {
      const draft = draftForPath(oldPath, get(draftsStore), this.vault);
      if (draft) {
        const draftCount = await this.countWordsInDraft(draft);
        draftWordCountsStore.update((counts) => {
          counts[draft.vaultPath] = draftCount;
  ...
  async debouncedCountDraftContaining(
    file: TAbstractFile,
